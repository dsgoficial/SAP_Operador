...
class LocalLayersWidget(QtWidgets.QWidget):

    dialog_path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), 
        'localLayersWidget.ui'
    )

    def __init__(self):
        super(LocalLayersWidget, self).__init__()
        ...
        self.searchs = [
            self.search_all_workspaces,
            self.search_selected_workspaces,
    ...
    def restart(self):
        for i in [self.call_all_btn_2, self.call_all_btn_1,  self.call_all_btn_3]:
    ...
    def load(self, data):
        ...
        self.workspaces_list.addItems(data['workspaces'])
    ...
    def get_inputs(self):
        ...
        workspaces = [ 
            self.workspaces_list_input.item(i).text() 
            for i in range(self.workspaces_list_input.count())
        ]
    ...
    def move_items(self, name=None):
        ...
        ignore = False
        if op == '1':
            origin, destination = ([self.workspaces_list, self.workspaces_list_input]
                if cmd == 'send' else [self.workspaces_list_input, self.workspaces_list]
            )    
        elif op == '2':
            if self.workspaces_list_input.count() > 0:
    ...
    def search_list(self, text):
        ...
        if option == 'layers':
            ...
            list_items = (
                self.workspaces_list if mode == 'all' 
                else self.workspaces_list_input
            )
        items = [
